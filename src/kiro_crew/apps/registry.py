...
async def install_from_registry(
    ...
    try:
        ...
        if not build_result["ok"]:
            ...
            # _clone_build_app), a failed clone, or a failed build may have
            # left a non-restorable origin-mismatch checkout moved aside. A
            # failure exit: the outer `finally` restores a restorable stale, so
            # filter it here; the non-restorable one is reported so it is not
            # stranded at `.stale-*` unlogged until the retention sweep.
        ...
        if subdirectory:
            ...
            if contained is None:
                ...
                # Report a retained non-restorable move-aside: the _unpoison
                # above restores only a restorable stale, so an origin-mismatch
                # one is stranded here. A failure exit (the `finally` restores a
                # restorable stale), so filter_restorable=True.
                _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=True)
                outcome = {
                    "ok": False,
                    "name": name,
        ...
        if manifest_data is None or str(manifest_data.get("name", "") or "") != name:
            ...
            # Report a retained non-restorable move-aside stranded by this
            # refusal (the `finally` re-stamps outcome["log"] from log_lines).
            # A failure exit, so filter the restorable stale the `finally`
            # restores.
            _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=True)
            return outcome
        ...
        if denied:
            ...
            # Report a retained non-restorable move-aside this refusal strands
            # (_unpoison restores only the restorable subset). A failure exit,
            # so filter the restorable stale the `finally` restores.
            _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=True)
            await _unpoison_rejected_checkout(
                name,
                app_source_dir(name),
        ...
        if install_script:
            ...
            if manifest_data is None or str(manifest_data.get("name", "") or "") != name:
                ...
                # Report a retained non-restorable move-aside stranded by this
                # post-script refusal. A failure exit, so filter the restorable
                # stale the `finally` restores.
                _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=True)
                return outcome
            denied = app_admission_denied(
                name,
            ...
            if denied:
                ...
                # Report a retained non-restorable move-aside this refusal
                # strands (_unpoison restores only the restorable subset). A
                # failure exit, so filter the restorable stale the `finally`
                # restores.
                _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=True)
                await _unpoison_rejected_checkout(
                    name,
