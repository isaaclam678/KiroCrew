...
async def _handle_create(request: web.Request) -> web.Response:
    ...
    if not import_existing:
        ...
        if not cleared and await _recorded_answers(str(spec_dir)):
            # The clear did not take AND the record is still readable, so proceeding
            # would hand this brand-new spec the previous one's answers -- the exact
            # outcome the clear above exists to prevent. Housekeeping was allowed to
            # fail on the DELETE path because the spec was already gone; here the
            # spec does not exist yet, so refusing costs the user a retry instead of
            # a spec whose cards are locked to answers they never gave.
            #
            # Gated on the record still READING back, not merely on the clear
            # failing. The other reason a clear fails is an unusable store -- and an
            # unusable store is one nothing can be read from either, so no stale
            # answer can reach a card. Refusing on that too would block every
            # create, including for directories that never had a record, in exchange
            # for no integrity at all.
...
async def _handle_stop_execution(request: web.Request) -> web.Response:
    ...
    dir_key = await _acanonical_dir(str(spec_dir))
    ...
    async with _turn_lock(dir_key):
        ...
        if await _acanonical_dir(str(spec_dir)) != dir_key:
