...
@pytest.mark.asyncio
async def test_create_proceeds_when_the_ledger_is_merely_unreadable(tmp_path, monkeypatch):
    """An unusable store fails the clear too, but nothing can be read from it.

    Refusing here would block creates for directories that never had a record, and
    buy no integrity: the overlay that renders a lock reads the same store, so an
    unreadable one cannot produce a false lock.
    """
    project = tmp_path / "proj"
    project.mkdir()
    client, state = _create_client(monkeypatch, tmp_path)
...
def test_stop_takes_the_directory_turn_lock():
    ...
    assert "_acanonical_dir(" in src, "Stop keys its lock on something other than the directory"
