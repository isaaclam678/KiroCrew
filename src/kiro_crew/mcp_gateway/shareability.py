...
def assess(evidence: ShareEvidence) -> ShareVerdict:
    ...
    # 3. Config-derived notes. Computed BEFORE the probe gate on purpose: these
    #    are facts about the config, true whether or not the server ever started,
    #    and reporting them is more useful than "unknown" on a host where the
    #    probe cannot run.
    #
    #    Both used to return DISQUALIFIED. Neither is a leak, and neither is even
    #    a fact about the world -- each is a gap on our own side:
    #
    #    * ``rotating_secret_env`` -- a secret-prefixed key is never forwarded into
    #      a SHARED backend (``gatewayd._declared_non_secret_env`` drops it;
    #      ``hashing.ENV_SCRUB_PREFIXES`` explains why: the keys are excluded from
    #      the pool hash so rotation cannot shatter the pool, which makes the hash
    #      non-injective over them, so no single value is correct). The pooled
    #      backend therefore receives NOBODY's secret rather than the wrong
    #      session's -- nothing crosses tenants. A private backend does get it
    #      (``_declared_env_for_private_backend``), so what pooling costs is a
    #      server that authenticates FROM declared env, and it fails loudly at its
    #      own auth layer. A server following the documented pattern -- read the
    #      credential from disk -- declares the key without needing it and pools
    #      fine. The durable answer is to route by credential identity (the secret's
    #      digest in the pool key, with ``credwatch`` already handling rotation by
    #      content digest), which retires this category rather than gating on it.
    #    * ``session_bound_by_construction`` -- ``managed_server_is_session_bound``
    #      returns False for anything not managed by Kiro Crew, by construction, so
    #      this NEVER fires on a third-party server. It is a list of our own
    #      unfinished work: ``kirocrew-core`` resolves the caller from the block
    #      gatewayd injects per call, ``kirocrew-cron`` still reads process env.
    #      The mechanism exists; that server has not adopted it. Withholding
    #      pooling from our own server protects nobody -- fixing the server does.
    #
    #    So both are reported and neither gates anything.
    notes: list[Reason] = []
    if evidence.session_bound_by_construction:
        notes.append(Reason("session_bound_by_construction"))
    ...
    if _declares_caller_identity(evidence.capabilities):
        ...
        reasons.append(
            Reason("preflight_passed" if evidence.preflight_ran else "preflight_not_run")
        )
