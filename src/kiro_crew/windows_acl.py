...
def _load() -> tuple[C.WinDLL, C.WinDLL]:
...
def _sid_to_string(advapi32: C.WinDLL, kernel32: C.WinDLL, psid: C.c_void_p) -> str:
    out = W.LPWSTR()
    if not advapi32.ConvertSidToStringSidW(psid, C.byref(out)):
        raise AclUnavailable(f"ConvertSidToStringSid failed (error {C.get_last_error()})")
    try:
        return out.value or ""
    finally:
...
def _sid_to_name(advapi32: C.WinDLL, psid: C.c_void_p) -> str:
...
def is_token_elevated() -> bool:
    ...
    if not advapi32.OpenProcessToken(kernel32.GetCurrentProcess(), TOKEN_QUERY, C.byref(token)):
        raise AclUnavailable(f"OpenProcessToken failed (error {C.get_last_error()})")
    try:
        elevation = W.DWORD()
        returned = W.DWORD()
        if not advapi32.GetTokenInformation(
            ...
            raise AclUnavailable(
                f"GetTokenInformation(TokenElevation) failed (error {C.get_last_error()})"
            )
    ...
    try:
        ...
        for index in range(dacl.contents.AceCount):
            ...
            if not advapi32.GetAce(dacl, index, C.byref(ace_pointer)):
                raise AclUnavailable(f"GetAce({index}) failed (error {C.get_last_error()})")
