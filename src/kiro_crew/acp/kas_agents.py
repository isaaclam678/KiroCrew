...
def to_client_custom_agent(
    ...
    # in KAS's vocabulary and forwarding it would be one line: `allowedTools` is
    # the only auto-approve input that has passed Crew's governance ceiling
    # (`_may_auto_approve`), so projecting a hand-written block would hand any
    # editor of the file a grant the ceiling never saw — and an auto-approved call
    # never reaches Crew's permission callback, so the deny-list and the audit
    # trail are skipped with it. One governed input, one derivation.
    #
    ...
    permissions = allowed_tools_to_permissions(spec.get("allowedTools"), agent_id=agent_id)
    if permissions:
