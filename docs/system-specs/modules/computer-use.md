...
- **A drag must name `global` explicitly.** `DragRequest.method` defaults to
  ...
  gate upstream reads.
- **A truncated `SendInput` batch releases the button and REPORTS.** `SendInput`
  returns how many records it accepted and can stop early — a lower-level hook, or
  UIPI blocking the injection mid-batch — and it returns **normally**, so nothing
  raises. A batch cut between a button-down and its button-up leaves the operator's
  own mouse button physically held, and nothing later un-holds it: every subsequent
  motion of their hand becomes a drag, which on a file manager moves their files and
  on a canvas destroys their work. So `post_mouse_click` and `post_mouse_drag` both
  compare the accepted count against the batch length, submit the release regardless,
  and raise — a silent partial delivery would otherwise be read as a successful click,
  and for a drag as a drop that landed. This is the mouse twin of `send_key_chord`'s
  unconditional reverse release, which was already safe by construction because it
  sends its whole `up` list in a `finally`. The `finally` alone is not enough here: it
  covers an exception, not a truncation.
