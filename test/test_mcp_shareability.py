...
class TestDisqualifiers:
    ...
    def test_a_session_bound_server_is_stubbed_but_not_auto_shared(self) -> None:
        """The note is the identity mechanism, not the authorship.
        ...
        sessions correctly. The sibling test below pins the other half -- that
        being ours is NOT itself a reason.

        No longer a disqualification either. gatewayd forwards no
        session-identifying env at all, so on a shared backend such a server reads
        EMPTY rather than another session's identity: its session-scoped features
        degrade, nothing crosses tenants. That is a named breakage, so it withholds
        the automatic share -- and nothing more. The stub still stands, because a
        stub is 1:1 with the session and changes nothing about how the server runs.
        """
        verdict = assess(
            ShareEvidence(
                name="kirocrew-cron",
                session_bound_by_construction=True,
                probe_ok=True,
                has_tools=True,
                capabilities={},
                preflight_ran=True,
        ...
        assert "session_bound_by_construction" in _codes(verdict)
...
class TestPositiveDeclaration:
    ...
    def test_a_divergence_does_not_beat_the_declaration(self) -> None:
        ...
        assert verdict.recommend_share is True
        assert "preflight_passed" in _codes(verdict), "the pass DID run"
