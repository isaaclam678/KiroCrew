...
class TestUnaffectedShapes:
    ...
    @pytest.mark.asyncio
    async def test_disk_rows_are_not_folded(self, state: Any) -> None:
        """The fold is scoped to the window tail, and must stay there.

        It runs on the event loop, so widening it over the chained history
        would put whole-history work back on the loop -- which is what the
        render offload exists to prevent. Persisted history carries no ``chunk``
        row to fold, and one that somehow appears is left to the render pass,
        matching the unbounded branch.

        The disk rows below hold a two-row run and the window carries one
        un-flushed row, so the fold branch does execute; `total` then
        distinguishes the two placements, because folding that disk run would
        shrink it by one.
        """
        disk = [
        ...
        assert data["total"] == len(disk) + 1
        # Still renders as one message per turn, because _prepare_messages
        # folds the un-folded disk run at render time.
