...
class TestTheWriteIsLockedAndOffLoop:
    ...
    async def test_materializing_a_workspace_entry_writes_no_directory(self, wired):
        """A materialized entry holds ONLY variables — deliberately no ``dir``.

        This branch is reachable only for a workspace ``config.json`` does not list,
        and a PUT requires membership in the MERGED view, so the workspace is
        supplied by ``config.local.json``. Writing a directory here would copy the
        overlay's value into the base file: removing the workspace from the overlay
        later would resurrect it from base with a stale path. The deep merge supplies
        the real directory on every load.

        The earlier version asserted the opposite (``entry["dir"] == "workspace-ops"``)
        and so pinned the leak in place.
        """
        _, path = wired
        ...
        assert "dir" not in entry, "an overlay-derived directory was copied into config.json"
