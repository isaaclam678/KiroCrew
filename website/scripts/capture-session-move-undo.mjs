...
try {
  ...
  // The keyboard path, which is the one the bar advertises.
  await page.keyboard.press('Control+z')
  await page.waitForTimeout(600)
  assert('4 undo: bar retired', (await bar.count()) === 0)
  assert('4 undo: session left Archive again', (await inFolder()) === 0)
  assert('4 undo: session still in the list', (await row.count()) === 1)
  ...
  // and a still frame can prove none of that, so the PR carries a GIF too.
  {
    ...
    let n = 0
    const frame = async () => {
      await p.screenshot({ path: `${OUT}/frames/${String(++n).padStart(2, '0')}.png`, clip })
    }
    await frame()
    ...
    console.log(`wrote ${n} flow frames to ${OUT}/frames`)
    console.log('assemble the committed GIF with:')
    console.log(`  python3 -c "from PIL import Image; import glob; fs=[Image.open(f) for f in sorted(glob.glob('${OUT}/frames/*.png'))]; fs[0].save('${OUT}/undo-flow.gif', save_all=True, append_images=fs[1:], duration=550, loop=0)"`)
  }
