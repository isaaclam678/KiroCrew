...
function SwitcherRow({
  ...
  // looks like the pin silently failed, and a dimmed glyph cannot say why.
  const pinTitle = pinned
    ? i18nT('components.instanceTabBar.unpin_crew', { name: entry.name })
    : i18nT('components.instanceTabBar.pin_crew', { name: entry.name })
  const pinLabel =
    pinned && noRoom ? `${pinTitle} — ${i18nT('components.instanceTabBar.no_room')}` : pinTitle
  return (
    ...
    <div className="flex items-center">
      ...
      >
        {/* Lit and filled = pinned, chip on screen. Lit but faded = pinned with
            its chip cut off. Unlit outline = not pinned, and it is the control's
            resting state rather than an absence, so the row shows where to click.
            The resting glyph carries no opacity: it is the only affordance the
            feature has, and `--muted` composited below full strength drops under
            the 3:1 contrast floor a UI control has to clear.
            The words are in the item's own accessible name. */}
        <Pin
