 ...
 */
export default function SessionMoveUndoBar({ moved, onUndo }: Props) {
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 border-t border-border bg-accent-subtle px-2.5 py-1.5" role="status" aria-live="polite">
        ...
        <span className="flex min-w-0 flex-1 items-center gap-1 text-[12px] text-text" title={moved.sessionTitle}>
          <span className="shrink-0">{i18nT('components.sessionMoveUndoBar.moved_to')}</span>
        ...
        >
          {i18nT('components.sessionMoveUndoBar.undo')} <span className="opacity-60">{platformShortcut('Cmd+Z')}</span>
        </button>
      </div>
