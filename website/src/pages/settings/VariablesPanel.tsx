...
interface TableProps {
  ...
  /**
   * Apply a PER-KEY change. `set` writes named pairs, `remove` deletes named keys;
   ...
   */
  onSave: (change: { set?: Record<string, string>; delete?: string[] }) => void
  busy: boolean
...
export function VariablesPanel() {
  ...
  const saveMut = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      // 4xx/5xx, and the shared `j()` helper THROWS on a non-OK response — so a
      // refusal never reaches onSuccess and a 200-with-error branch would be dead
      // code. `variables_overlay_owned` is the case that makes this matter: it is
      // the one refusal client-side validation cannot pre-empt, because a key owned
      // only by config.local.json is absent from `pairs` and so never renders as a
      // row for a duplicate check to catch. Without this the user would see the
      // generic message and no reason.
      ...
      <SettingsSection title={i18nT('pages.settings.variablesPanel.global_environment_variables')}>
        <SettingsCard>
          ...
          <VariableTable
            ...
            busy={busy}
            noteId={noteId}
            onSave={change => saveMut.mutate({ scope: 'global', ...change })}
                ...
                return (
                  ...
                  <div key={ws} role="region" aria-label={i18nT('pages.settings.variablesPanel.workspace_named', { name: ws })} className="flex flex-col gap-2">
                    ...
                    <VariableTable
                      ...
                      busy={busy}
                      noteId={noteId}
                      onSave={change => saveMut.mutate({ scope: 'workspace', workspace: ws, ...change })}
