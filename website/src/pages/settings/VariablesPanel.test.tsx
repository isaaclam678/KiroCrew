...
describe('VariablesPanel', () => {
  ...
  describe('global scope', () => {
    ...
    it("surfaces the backend's reason for a refusal, not a generic failure", async () => {
      // The real shape: every refusal is a 4xx and the shared `j()` helper THROWS
      // on a non-OK response, carrying the body as `detail`. An earlier version of
      // this test asserted a 200-with-error payload the backend never sends, so it
      // was exercising a dead branch. `variables_overlay_owned` is the refusal that
      // makes this matter — client-side validation cannot pre-empt it, because a key
      // owned only by config.local.json never renders as a row.
      variables.mockResolvedValue(view())
      saveVariables.mockRejectedValue({
        status: 400,
        detail: JSON.stringify({
          error: 'these variables come from config.local.json and cannot be changed here: REGION',
    ...
    it('falls back to the generic message when the body is not JSON', async () => {
      variables.mockResolvedValue(view())
      saveVariables.mockRejectedValue({ status: 500, detail: '<html>gateway error</html>' })
      mount()
      await ready()
