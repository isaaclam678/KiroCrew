...
import { Rocket, Menu, Bell, Code, RefreshCw, Package, Loader2, Download, Hammer, XCircle, Check, AlertTriangle, CheckCircle, X, AudioWaveform, ChevronUp, MoreHorizontal, Coins, ArrowLeftToLine, LayoutGrid, SquareTerminal, Bot, Maximize2, Minimize2, Search as SearchIcon } from 'lucide-react'
...
export default function App() {
  ...
  const topPeek = useHoverIntent({ enabled: focusActive, openMs: 120, closeMs: 260, triggerRef: topPeekTrigger, surfaceRef: topPeekSurface })
  const railPeek = useHoverIntent({ enabled: focusActive, openMs: 120, closeMs: 260, triggerRef: railPeekTrigger, surfaceRef: railPeekSurface })
  ...
  // Height the shell's first row keeps while focus mode is on.
  //
  // NOT zero: the content row's own cards (the chat sessions drawer's `pb-2`,
  // the side panel's and nav rail's `mb-2`) are inset 8px at the BOTTOM and zero
  // at the top, because the 42px top bar row was what gave them their top
  // clearance. Collapsing that row outright left them hugging the window's top
  // edge while still inset at the bottom — visibly lopsided. 8px reproduces the
  // spacing they already carry, so focus mode reclaims the chrome without
  // changing how the surfaces themselves sit.
  //
  // macOS-windowed Electron used to keep the full 42px here so the native
  // traffic lights had somewhere to sit. That reclaimed nothing and clipped the
  // sessions sidebar and the chat title row anyway, because AppKit paints those
  // buttons at a WINDOW coordinate and they do not move with the DOM. They are
  // hidden with the header instead (see `focusChromeVisible`), so every platform
  // now gets the same 8px and there is nothing left to reserve for.
  const focusTopInset = FOCUS_INSET
  ...
  return (
    ...
    <div
      ...
      style={{
        ...
        // so the one platform that needs a gutter (see focusTopInset) can keep it.
        ...(focusActive && {
          gridTemplateRows: bottomDock
            ? `${focusTopInset}px minmax(0,1fr) auto`
            : `${focusTopInset}px minmax(0,1fr)`,
        }),
      }}
    >
      ...
      >
        ...
        {!isMobile && (
          ...
          <button
            ...
            title={`${i18nT('app.focus_mode')} · ${IS_MAC ? '⌥⇧M' : 'Alt+Shift+M'}`}
          >
