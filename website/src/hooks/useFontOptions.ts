...
export interface FontOptionsState {
   ...
   *
   * `added` needs no message — the list visibly grows. The others are the states
   * that otherwise look like the click did nothing: the wait while the browser
   * decides, a refused prompt, and a grant that turned up nothing the probe had
   * not already found.
   */
